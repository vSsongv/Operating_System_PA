...
static struct process *sjf_schedule(void)
{
	...
	if(!list_empty(&readyqueue))
		{
			list_for_each_entry(sjfProc, &readyqueue, list)
			{
...
static struct process *srtf_schedule(void)
{
	...
	if(current)
		{	
			if(!list_empty(&readyqueue))
			{
				list_for_each_entry(srtfProc, &readyqueue, list)
				{
					if(srtfProc->lifespan < lsChk)
					{
            	    	lsChk = srtfProc->lifespan;
				...
				if(current->lifespan > srtfSmallest->lifespan)
				{
					...
					list_add(&nsrtfProc->list, &readyqueue);
	...
	if(current)
    	{
			if(!list_empty(&readyqueue))
			{
				...
				{
    			    if(prioProc->prio >= prioChk)
					{
						prioChk = prioProc->prio;
						highest = prioProc;
					}
    			}

				if(current->prio < highest->prio)
				{
					nprioProc = current;
					current = highest;
					list_add_tail(&nprioProc->list,&readyqueue);
					list_del_init(&current->list);
					return current;
				}
