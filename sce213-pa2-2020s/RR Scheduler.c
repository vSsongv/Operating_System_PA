...
{
	struct process *sjfChk = list_first_entry(&readyqueue, struct process, list);
	...
	if(!list_empty(&readyqueue))
		{
			list_for_each_entry(sjfChk, &readyqueue, list)
			{
				if(sjfChk->lifespan < temp)
				{
                	temp = sjfChk->lifespan;
					sjfSmallest = sjfChk;
				}
			}
...
{
	struct process *srtfChk = list_first_entry(&readyqueue, struct process, list);
	...
	int lsChk = 100;
	 if(current)
		{	
			if(!list_empty(&readyqueue))
			{
				list_for_each_entry(srtfChk, &readyqueue, list)
				{
					if(srtfChk->lifespan < lsChk)
					{
            	    	lsChk = srtfChk->lifespan;
						srtfSmallest = srtfChk;
					}
				}

				if(current->lifespan > srtfSmallest->lifespan)
				{
					struct process *temp = NULL;
					temp = current;
					current = srtfSmallest;
					list_add(&temp->list, &readyqueue);
