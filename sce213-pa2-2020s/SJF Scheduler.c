...
static struct process *sjf_schedule(void)
{
	...
	check = list_first_entry(&readyqueue, struct process, list);
	struct process *smallest = NULL;
	...
	if(!list_empty(&readyqueue))
		{	
				list_for_each_entry(check, &readyqueue, list)
				{
					if(check->lifespan < temp)
					{
                    	temp = check->lifespan;
						smallest = check;
					}
...
static struct process *sjrf_schedule(void)
{
