...
void make_dir_and_file(directory_t* root_dir, char** token_list, int num_token) 
{
	...
	directory_t* parent = root_dir;
	directory_t* T_dir;
	file_t* file;

	for (int i = 0; i < num_token - 1; i++)
	{
		T_dir = find_create_dir(parent, *(token_list + i), &is_create);
		if (is_create == true)
		{
			parent->dir_list[parent->num_dir] = T_dir;
			parent->num_dir++;
		}
		parent = T_dir;
		is_create = false;
	...
	file = find_create_file(parent, *(token_list + (num_token - 1)), &is_create);
	
	if (is_create == true)
	{
		parent->file_list[parent->num_file] = file;
		parent->num_file++;
	}
	return;
}
